  ...
  function(oj, ko, jsonData, moment) {
    ...
    function PersonViewModel() {
      ...
      self.orgPeopleDataSource = ko.computed(function() {
        return new oj.ArrayTableDataSource(self.directReports(), {
          idAttribute: 'empId'
        });
      });
      ...
      self.findAvgRating = function(emp) {
        var avgRate, avgPot, sumRate = 0,
          sumPot = 0,
          rateCount = emp.perfs.length;
      ...
      function loadPerfandPotenialData() {
        ...
        self.pieSeriesValue = ko.observableArray(
          [{
              name: "Bonus",
              items: [data.comps[0].bonus]
            },
            {
              name: "Comission",
              items: [data.comps[0].comission]
            },
            {
              name: "Salary",
              items: [data.comps[0].compSalary]
            }
          ]
        );
        loadTeamData(data);
      ...
      function loadTeamData(data) {
        ...
        self.infoTiles([{
            ...
            "title": "Summary",
            "infolable1": "Skills",
            "infolable1value": data.skills.length,
            "infolable2": "Tenure",
            "infolable2value": self.getTenure(data)
          },
          {
            "sid": "2",
            "name": "Item2",
            "title": "Performance",
            "infolable1": "Rating",
            "infolable1value": data.rating,
            "infolable2": "Potential",
            "infolable2value": data.potential
          },
          {
            "sid": "3",
            "name": "Item3",
            "title": "Compensation",
            "infolable1": "Salary",
            "infolable1value": "$" + (data.salary > 999) ? (data.salary / 1000).toFixed(0) + 'k' : data.salary,
            "infolable2": "Ratio",
            "infolable2value": data.compRatio
          },
          {
            "sid": "4",
            "name": "Item4",
            ...
            "infolable1value": data.skills.length,
            "infolable2": "Directs",
            "infolable2value": self.directReports().length
          }
        ]);
        self.infoTilesDataSource = new oj.ArrayTableDataSource(self.infoTiles(), {
          idAttribute: 'sid'
        });
